...
**The implicit control holds.** Mycenaean returns −0.90: Linear A is known not to be Greek, and the instrument places Greek in thegroup with no signal. Had it fired, the rest of the table would be worthless.
...
That number is the explanation of the century described in §1. It is not that no proposal is serious. It is that with 351 bits of slack, any sufficiently worked hypothesis produces a coherent lexicon. Packard demonstrated in 1974 that nine fictitious decipherments could be built; this is the figure that explains why he could.
