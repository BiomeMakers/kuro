...
**Alberto Acedo**
Biome Makers Inc. Draft v0.1, 11 September 2026. Not for circulation.
...
## 4 bis. Which archive each function was fixed on

The 56 units were fixed on the whole corpus, but the corpus is Haghia Triada in 64%, and the archives differ. Khania, the second usable series (227 documents, 105 tablets), carries no term of summation on any of its tablets, against 41 of 205 at Haghia Triada (binomial at the HT rate: p = 7·10⁻¹¹). Its dominant commodity is cyperus, 68% of its records carry a fraction (36% at HT), and its tablets are half as long. It is an archive of small-scale distribution against a bulk store.

Two warnings follow for the use of the table above. First: the arithmetic functions (KU-RO, KI-RO, PO-TO-KU-RO, DA-I) are functions **of the Haghia Triada archive**, and their absence at Khania is not a lack of material. Second: the qualifiers and headings were fixed with base rates of the whole corpus; measured with Khania's own rates, that archive contributes no new one, and the only one that survives there (PA) comes from Haghia Triada. The exception is CYP+D, which only the Khania archive allows one to fix: it carries the same quantity in 12 of its 16 quantified attestations (base rate 22%, p = 10⁻⁵) and is issued after entries of persons, the one quantity in the corpus that behaves as a fixed allocation.
