...
**Closed, with a null:** that the consonant phonotactics of Minoan under the current transcription resembles any of thirteen candidates. The three historical ones — Luwian, Hurrian, Hattic — are measured against a null here for the first time, and none passes.

**Not closed:** the affiliation of Minoan. Four readings remain compatible with everything measured: that Minoan is an isolate; that it is related to a language not in the set; that the Greek filter deforms the phonotactics too much for any signal to survive; or that the Linear B values are wrong for Linear A, in which case what was measured is the phonotactics of an incorrect transcription. The last two are the same obstacle seen from two sides, and they are what any further phonological comparison must address before adding candidates. Adding candidates is not the remedy: fifty more would return fifty more p-values near 1.00 for the same reason.
